...
export type RegisterResolver = (parent: any, {email, password} : {email: string, password: string, display: string, bio: string, img: any, zoom: number, offsetX: number, offsetY: number, width: number, height: number}, context: ApolloContext, info: any) => Promise<User|null>;
